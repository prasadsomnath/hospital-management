...
export default function ManagePharmacists() {
  ...
  const fetchPharmacists = async () => {
    ...
    try {
      ...
      const storedMetadata = localStorage.getItem("medicore-pharmacists-meta");
      const metadataMap = storedMetadata ? JSON.parse(storedMetadata) : {};

      const mapped = credentials.map((c: any): Pharmacist => {
      ...
      // fallback to local stored pharmacists
      const storedMetadata = localStorage.getItem("medicore-pharmacists-meta");
      const metadataMap = storedMetadata ? JSON.parse(storedMetadata) : {};
      const localPharmacists = Object.keys(metadataMap).map(
        (email, idx): Pharmacist => {
      ...
      setPharmacists(localPharmacists);
      setTotalPages(1);
      setTotalElements(localPharmacists.length);
    } finally {
      setLoading(false);
    }
  };
  ...
  function openEdit(p: Pharmacist) {
    setEditingId(p.id);
    setForm({
      name: p.name,
      email: p.email,
  ...
  async function handleSave() {
    ...
    try {
      if (editingId) {
        ...
        const entityId = `PHARM-${Date.now().toString().substring(5)}`;
        ...
        toast.success("Pharmacist registered successfully on backend!");
      }
      setDialogOpen(false);
      setForm(EMPTY_FORM);
      fetchPharmacists();
      ...
      if (
        ...
        try {
          ...
          if (editingId) {
            ...
            const newEntityId = `PHARM-LOCAL-${Date.now().toString().substring(5)}`;
            updatedCreds.push({
              email: form.email,
              password: form.password || "password",
              role: "pharmacist",
      ...
      let userFriendlyMsg = e.message || "Failed to save pharmacist";
      if (
  ...
  async function handleDelete(id: string) {
    ...
    try {
      ...
      console.error(e);
      toast.error(e.message || "Failed to delete pharmacist");
    }
  }
  ...
  async function handleUpdateStatus(email: string, activate: boolean) {
    try {
      ...
      if (
        ...
        toast.success(
          `Pharmacist status updated to ${activate ? "Active" : "Inactive"} (offline)!`,
        );
        fetchPharmacists();
        return;
      }
      toast.error(e.message || "Failed to update pharmacist status");
    }
  }

  return (
    <div className="space-y-6" data-ocid="admin.pharmacists.page">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-0.5">
            {totalElements} registered pharmacists for hospital{" "}
            {user?.hospitalCode}
          </p>
        </div>
        <Button
        ...
        >
          <Plus className="h-4 w-4" /> Add Pharmacist
        </Button>
      </div>
            ...
            {loading ? (
              ...
              <tr>
                <td
                  ...
                  className="text-center py-8 text-muted-foreground"
                >
                  No pharmacists found matching your search.
                </td>
              </tr>
            ) : (
                    ...
                    <div className="flex justify-end gap-2">
                      {p.status === "Inactive" ? (
                        <Button
                          size="icon"
                          variant="ghost"
                          onClick={() => handleUpdateStatus(p.email, true)}
                          className="h-8 w-8 text-muted-foreground hover:text-emerald-500"
                          title="Activate Pharmacist"
                        >
                        ...
                        <Button
                          size="icon"
                          variant="ghost"
                          onClick={() => handleUpdateStatus(p.email, false)}
                          className="h-8 w-8 text-muted-foreground hover:text-amber-500"
                          title="Deactivate Pharmacist"
                        >
          ...
          <div className="space-y-4 mt-2">
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2 space-y-1.5">
                <Label htmlFor="pharm-name">Full Name</Label>
                <Input
                  id="pharm-name"
                  value={form.name}
            ...
            <div className="flex justify-end gap-3 pt-2">
              ...
              >
                {editingId ? "Save Changes" : "Add Pharmacist"}
              </Button>
            </div>
          </div>
        </DialogContent>
      ...
      <Dialog open={!!deleteId} onOpenChange={(o) => !o && setDeleteId(null)}>
        <DialogContent className="bg-card border-border max-w-sm">
          ...
          <p className="text-muted-foreground text-sm mt-2">
            Are you sure you want to remove this pharmacist? This action cannot
            be undone.
          </p>
